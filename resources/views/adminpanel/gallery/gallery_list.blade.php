<table class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline" id="sample_1" role="grid"
    ...
    <tr role="row">
        ...
        <th class="" tabindex="0" aria-controls="sample_1" rowspan="1" colspan="1" data-column-index="1"> Image</th>
    ...
    @if(is_array($result) && count($result) > 0)
        @foreach ($result as $r)
            ...
            <tr role="row">
                <td> {{$i}} </td>
                <td>
                    <a href="{{asset('uploads/gallery/'.$r->image)}}" target="_blank">
                        <img src="{{asset('uploads/gallery/'.$r->image)}}" width="150" />
                    </a>
                </td>
                <td>
                    @if($r->status == 'Y')
                ...
                <td>
                    <a onClick="return confirm('Are you sure you want to update?');" href="{{url('edit_gallery', $r->id)}}" class="btn btn-info" id="btn-view"><i class="fa fa-edit"></i> Edit</a>
                    <a onClick="return confirm('Are you sure you want to delete?');" href="{{url('delete_gallery', $r->id)}}" class="btn btn-danger"><i class="fa fa-trash"></i> Delete</a>
                </td>
            </tr>
            @php
                $i++;
